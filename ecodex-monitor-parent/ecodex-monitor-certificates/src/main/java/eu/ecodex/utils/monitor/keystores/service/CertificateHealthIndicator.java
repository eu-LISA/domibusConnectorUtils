...
public class CertificateHealthIndicator extends AbstractHealthIndicator {
    ...
    private void keyCheck(KeyCheck check, Health.Builder builder) {
        ...
        if (!storeEntryInfo.getPresent()) {
            builder.unknown();
            builder.withDetail(checkName + "_message",
                    String.format("No certificate found in store %s with alias %s",
    ...
    private void checkValidation(KeyCheck check, String checkName, StoreEntryInfo storeEntryInfo,
...
//            }
    ...
    private void checkNotAfter(KeyCheck check, String checkName, StoreEntryInfo storeEntryInfo,
                               Health.Builder builder) {
        ...
        if (storeEntryInfo.getNotAfter() == null) {
            builder.withDetail(checkName + "_not_after_message",
                    "Was not able to check not before, because there is no not before information available");
            return;
        }
    ...
    private void checkNotBefore(String checkName, StoreEntryInfo storeEntryInfo,
                                Health.Builder builder) {
        builder.withDetail(checkName + "_not_before_date", storeEntryInfo.getNotBefore());
        if (storeEntryInfo.getNotBefore() == null) {
            builder.withDetail(checkName + "_not_before_message",
                    "Was not able to check not before, because there is no not before information available");
